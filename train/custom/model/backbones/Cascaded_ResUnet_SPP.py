...
class ResUnet_SPP(nn.Module):
    ...
    def __init__(self, in_ch, channels=16, blocks=3):
        ...
        self.mconv1 = DoubleConv(channels * 3, channels * 1)
    ...
    def forward(self, input):
        ...
        merge3 = self.mconv3(torch.cat([self.up4(c4), c3], dim=1))
...
class Cascaded_ResUnet_SPP(nn.Module):
    def __init__(self, in_ch, channels=12, blocks=3):
        ...
        self.Unet1 = ResUnet_SPP(in_ch, channels, blocks)
        self.merge1 = DoubleConv(channels * 4, channels)
        self.Unet2 = ResUnet_SPP(channels, channels, blocks)
        self.merge2 = DoubleConv(channels * 4, channels)
    def forward(self, input):
        out1 = self.merge1(self.Unet1(input))
        out2 = self.merge2(self.Unet2(out1))
        return out1, out2 
